...
const DeleteForm = ({ data, setOpen, setRefresh }) => {
  const onSubmit = async () => {
    try {
      await axios.delete(
        `https://cafeteria-op-src-api.onrender.com/api/brands/${data.id}}`
      )
      setOpen(false)
  ...
  return (
    <Box>
      <BasicLabel
        label={`Estas seguro que desas eliminar la marca "${data.description}"?`}
        style={{ fontSize: 18 }}
      />
