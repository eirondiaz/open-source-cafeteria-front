...
const SupplierTable = (props) => {
  ...
  const onDelete = async () => {
    try {
      await axios.delete(
        `https://cafeteria-op-src-api.onrender.com/api/suppliers/${selectedSupplier.id}`
      )
  ...
  const columns = [
    {
      ...
      selector: (campus) => campus.comercialName,
      width: 130,
    },
    {
      name: 'RNC',
      selector: (campus) => campus.rnc,
      width: 130,
    },
    {
      name: 'Fecha creación',
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <BasicDialog
        target={'delete'}
        title={`Estas seguro que desas eliminar el suplidor ${
